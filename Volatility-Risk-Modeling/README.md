...
> `sympy` was used separately, outside this notebook, to symbolically verify the Christoffersen likelihood formula during development — it's not a runtime dependency and isn't imported by the notebook itself, so it's not required to run anything here.
